...
//ZVAR( double, Aode2_noise, 1e-4 );
ZVAR( double, Aode2_noise, 5.6e-3 );
//ZVAR( double, Aode2_noise, 0.0 );

ZVAR( int, Aode2_plotOGWRT, 1 );
ZVAR( int, Aode2_autoscale, 0 );
ZVARB( int, Aode2_plotDebugTraces, 1, 0, 1 );
...
ZFitterKin mainFitter;
ZFitterKin renderFitter;

ZFitterThread fitterThread;
...
struct ZUIFitterStatusText : ZUIText {
	...
	virtual void render() {
		...
		//ZFitterThread *fitThread = (ZFitterThread *)getI( "fitter" );
		ZFitterThread *fitThread = &fitterThread;
		if( fitThread ) {
			fitThread->lock();
			ZFitterKin *fitter = (ZFitterKin *)fitThread->renderCopy;
			if( fitter ) {
				static char *fitStatusText[] = { "Stopped", "Converged", "Trapped", "Running" };
				sprintf(
					buffer, "Status: %s\npropCount: %d\norigCount: %d\nError: %.2lg\nbackoff: %d\ncountSinceOrig: %d\npropEvalTime: %.2lg\norigNewTime: %.2lg\n",
					fitThread->threadPaused ? "Paused" : fitStatusText[ fitter->state ], 
					fitter->propCount(),
					fitter->origCount(),
					fitter->propE,
					fitter->propBackoffStepsOnLastEigenValue,
					fitter->propCountSinceOrig,
					fitter->timingPropEvalAccum,
					fitter->timingOrigNewAccum
				);

				strcat( buffer, "Reaction rates:\n" );

				for( r=0; r<fitter->system.reactionCount(); r++ ) {
					strcat( buffer, ZTmpStr( "  %d: %-.2lf\n", r, fitter->system.paramGetReactionRate(r) ) );
				}
			}
			fitThread->unlock();
		}
...
//		ZFitterThread *fitThread = (ZFitterThread *)getI( "fitter" );
					...
					for( int q=0; q<qCount; q++ ) {
						...
						if( q == whichDragging ) {
							if( getF( "lastTwiddle" ) != mouseY ) {
								...
								zMsgQueue( "type=FitterEigenTwiddle fitterKin=0x%x param=%d value=%f deltaValue=%f", (int)fitter, q, s, delta );
							}
	...
	void handleMsg( ZMsg *msg ) {
		if( zmsgIs(type,ZUIMouseClickOn) && zmsgIs(dir,D) && zmsgIs(which,L) ) {
			...
			//ZFitterThread *fitter = (ZFitterThread *)getI( "fitter" );
			ZFitterThread *fitter = &fitterThread;
			if( fitter ) {
				int qCount = fitter->renderCopy->qBeingFitCount;
				float myW, myH;
				getWH( myW, myH );
				float colWidth = qCount > 0 ? (myW-2.f*colSpace) / qCount - colSpace : 0.f;
...
struct ZUIFitterParameterAdjustPlot : ZUIPanel {
	...
	virtual void render() {
		...
		if( fitThread ) {
			...
			if( fitter ) {
				...
				int reactionCount = fitter->system.reactionCount();

				float y = myH - 15.f;
				glColor3ub( 255, 255, 255 );
				zglFontPrint( "Reactions", -20.f, y, "controls" );
				glColor3ub( 0, 0, 0 );
				...
				for( r=0; r<reactionCount; r+=2 ) {
					...
					if( twiddling ) {
						...
						glColor3ub( 0, 200, 0 );
						zglFontPrint( ZTmpStr("%4.1lf (%+4.1lf)", fitter->system.paramGetReactionRate(r+0)+x0, x0 ), x, y+18.f, "controls" );
						zglArrow( 25.f, y+12.f, 25.f+scale*x0, y+12.f, 0.f, 0.f, 2.f );

						if( r+1 < reactionCount ) {
							glColor3ub( 200, 0, 0 );
							zglFontPrint( ZTmpStr("%4.1lf (%+4.1lf)", fitter->system.paramGetReactionRate(r+1)+x1, x1 ), x, y-14.f, "controls" );
						...
						float x = 10.f;
						glColor3ub( 0, 200, 0 );
						zglFontPrint( ZTmpStr("%4.1lf", fitter->system.paramGetReactionRate(r+0)), x, y+18.f, "controls" );
						zglArrow( x+5.f, y+12.f, x+30.f, y+12.f, 0.f, 2.f, 2.f );

						if( r+1 < reactionCount ) {
							glColor3ub( 200, 0, 0 );
							zglFontPrint( ZTmpStr("%4.1lf", fitter->system.paramGetReactionRate(r+1)), x, y-14.f, "controls" );
					...
					for( int side=0; side<2; side++ ) {
						ZTmpStr str( "%s%s%s",
							fitter->system.reactionGet(r,side,0) == -1 ? "" : fitter->system.reagentGetName( fitter->system.reactionGet(r,side,0) ),
							fitter->system.reactionGet(r,side,0) != -1 && fitter->system.reactionGet(r,side,1) != -1 ? " + " : "",
							fitter->system.reactionGet(r,side,1) == -1 ? "" : fitter->system.reagentGetName( fitter->system.reactionGet(r,side,1) )
						);
				...
				int eCount = fitter->system.eCount();
				int reagentCount = fitter->system.reagentCount();
				y -= 40.f;

				if( twiddling ) {
					// DRAW the delta bars for ICs
					float _y = y - 18.f;
					for( int e=0; e<eCount; e++ ) {
						for( r=0; r<reagentCount; r++ ) {
							int q = (fitter->system.paramGet( PI_INIT_COND, 0, e )[r]).qIndex;

							float scale = 100.f;
							float x0 = (float)deltaParamVec.getD( q, 0 );

							glColor3ub( 208, 202, 27 );
							zglArrow( 25.f, _y+12.f, 25.f+scale*x0, _y+12.f, 0.f, 0.f, 2.f );
							_y -= 14.f;
						}
					}
				}
				...
				for( int e=0; e<eCount; e++ ) {
					for( r=0; r<reagentCount; r++ ) {
						glColor3ub( 0, 0, 0 );
						zglFontPrint( ZTmpStr("Ex %d: %s %4.1lf", e, fitter->system.reagentGetName(r), fitter->system.paramGetIC(e,r)), 0.f, y, "controls" );
						y -= 14.f;
					}
				}

				if( twiddling ) {
					// DRAW the delta bars for observable constants
					float _y = y - 58.f;
					int obsConstCount = 0;
					KineticParameterInfo *obsConsts = fitter->system.paramGet( PI_OBS_CONST, &obsConstCount );
					for( r=0; r<obsConstCount; r++ ) {
						int q = obsConsts[r].qIndex;
				...
				int obsConstCount = 0;
				KineticParameterInfo *pInfo = fitter->system.paramGet( PI_OBS_CONST, &obsConstCount );
				y -= 40.f;
				glColor3ub( 255, 255, 255 );
	...
	void handleMsg( ZMsg *msg ) {
		if( zmsgIs(type,FitterEigenTwiddle) ) {
			ZFitterKin *fitterKin = (ZFitterKin *)zmsgI(fitterKin);

			ZMat deltaSignal;
			deltaSignal.alloc( fitterKin->origSignal.rows, fitterKin->origSignal.cols, zmatF64 );
//			double sign = zmathSignDD( fitter->fitSignal.getD( zmsgI(param), 0 ) );
			deltaSignal.putD( zmsgI(param), 0, (double)zmsgF(deltaValue) );

			zmatMul( fitterKin->origVtt, deltaSignal, deltaParamVec );
	...
	virtual void render() {
		...
		if( fitThread ) {
			...
			if( fitter ) {
				...
				int eo = 0;
				for( int e=0; e<fitter->system.eCount(); e++ ) {
					for( int o=0; o<fitter->system.experiments[e]->observableResidualFFTMat.rows; o++, eo++ ) {
						glColor3fv( colors[eo] );
						glBegin( GL_LINE_STRIP );
						int cols = fitter->system.experiments[e]->observableResidualFFTMat.cols;
						for( int i=0; i<cols; i++ ) {
							float x = (float)i * myW / (float)cols;
							float y = (float)fitter->system.experiments[e]->observableResidualFFTMat.getD( o, i );
...
// @TODO: puysh pop these
	...
	for( e=0; e<renderFitter.system.eCount(); e++ ) {
		for( o=0; o<renderFitter.system.experiments[e]->observablesCount(); o++ ) {
			plots[plotCount++] = new TracePlot( &renderFitter.system.experiments[e]->traceOC, o, o,  KTP_STYLE_SLERP | KTP_STYLE_LINE_SOLID | KTP_STYLE_POINTS_BLACK_DOTS | KTP_STYLE_LINE_HALO, ZTmpStr("Fit Ex %d: Obs: %d",e,o) );
		}

		for( i=0; i<renderFitter.system.experiments[e]->traceC.rows; i++ ) {
			plots[plotCount++] = new TracePlot( &renderFitter.system.experiments[e]->traceC, i, i,  KTP_STYLE_SLERP | KTP_STYLE_LINE_DASHED | KTP_STYLE_POINTS_BLACK_DOTS, ZTmpStr("Fit Ex %d: traceC[%d]",e,i) );
		}

//		if( Aode2_plotOGWRT >= 0 && Aode2_plotOGWRT < renderFitter.system.experiments[e]->traceG.count ) {
//			int gCount = renderFit.system.experiments[e]->traceOG.count;
//			if( gCount >= 0 ) {
//				for( j=0; j<renderFitter.system.experiments[e]->traceOG[Aode2_plotOGWRT % gCount]->rows; j++ ) {
//					plots[plotCount++] = TracePlot( renderFitter.system.experiments[e]->traceOG[Aode2_plotOGWRT % gCount], j, j,  KTP_STYLE_LINE_DOTTED | KTP_STYLE_POINTS_BLACK_DOTS, ZTmpStr("Fit Ex %d: (traceG wrt %d)[%d]",e,Aode2_plotOGWRT,j) );
//				}
//			}
//		}

//		if( Aode2_plotDebugTraces ) {
//			for( i=0; i<renderFitter.system.experiments[e]->traceDebug.rows; i++ ) {
//				plots[plotCount++] = new TracePlot( &renderFitter.system.experiments[e]->traceDebug, i, i,  KTP_STYLE_SLERP | KTP_STYLE_LINE_DOTDASHED, ZTmpStr("Fit Ex %d: debug[%d]",e,i) );
//			}
//		}
	}
	...
	if( Aode2_autoscale ) {
		...
		for( i=0; i<plotCount; i++ ) {
			if( Aode2_autoscale!=2 || plots[i]->flags & KTP_STYLE_LINE_SOLID ) {
...
int setupTest28( int modify ) {
	setupCallback = setupTest28;
	// A->B, 1 experiments, 1 observables, modify 1 rate consts, fit 1 rate const
	// Test of exponential anlytic fit #1

	if( modify ) {
//		testSystem.paramSetReactionRate( 0, startSystem.paramGetReactionRate(0) * 0.5 );
//		testSystem.paramGetByName( "A" )->value *= 0.5;
//		testSystem.paramGetByName( "c" )->value *= 0.5;

		testSystem.paramSetReactionRate( 0, startSystem.paramGetReactionRate(0) * 0.0 );
		testSystem.paramGetByName( "A" )->value *= 0.0;
		testSystem.paramGetByName( "c" )->value *= 0.0;
	...
	e->observableInstructions.add( strdup("A+c") );

	testSystem.allocParameterInfo();
	testSystem.compile();

	testSystem.paramSetReactionRate( 0, 10 );
	testSystem.paramGetByName( "A" )->value = 1.0;
	testSystem.paramGetByName( "B" )->value = 0.0;
	testSystem.paramGetByName( "c" )->value = 1.0;

	testSystem.clrFitFlags();
	KineticParameterInfo *pi = testSystem.paramGet( PI_REACTION_RATE );
	pi->fitFlag = 1;
	pi = testSystem.paramGetByName( "A" );
	pi->fitFlag = 1;
	pi = testSystem.paramGetByName( "c" );
	pi->fitFlag = 1;
	return 0;
}

int setupTest29( int modify ) {
	setupCallback = setupTest29;
	// A->B, 1 experiments, 1 observables, modify 1 rate consts, fit 1 rate const
	// Test of exponential anlytic fit #1 but starting at zero

	if( modify ) {
		testSystem.paramSetReactionRate( 0, 0.0001 );
		testSystem.paramGetByName( "A" )->value = 0.0001;
		testSystem.paramGetByName( "c" )->value = 0.0001;
		return ZF_DONE_CONVERGED;
	}
...
int setupTest30( int modify ) {
	setupCallback = setupTest30;
	// A->B, 1 experiments, 1 observables, modify 1 rate consts, fit 1 rate const
	// Test of exponential anlytic fit #2

	if( modify ) {
		testSystem.paramSetReactionRate( 0, startSystem.paramGetReactionRate(0) * 0.5 );
		testSystem.paramSetReactionRate( 1, startSystem.paramGetReactionRate(1) * 0.5 );
		testSystem.paramGetByName( "A" )->value *= 0.5;
		testSystem.paramGetByName( "D" )->value *= 0.5;
		testSystem.paramGetByName( "c" )->value *= 0.5;
		return ZF_DONE_CONVERGED;
	}
	...
	testSystem.reagentAdd( "A" );
	testSystem.reagentAdd( "B" );
	testSystem.reagentAdd( "D" );
	testSystem.reagentAdd( "E" );

	testSystem.reactionAdd(  0, -1,  1, -1 );
	testSystem.reactionAdd(  2, -1,  3, -1 );

	KineticExperiment *e = testSystem.newExperiment();
	e->mixsteps[0].duration = 3.0;

	e->observableInstructions.add( strdup("A+D+c") );

	testSystem.allocParameterInfo();
	testSystem.compile();

	testSystem.paramSetReactionRate( 0, 10 );
	testSystem.paramSetReactionRate( 1, 5 );
	testSystem.paramGetByName( "A" )->value = 1.0;
	testSystem.paramGetByName( "B" )->value = 0.0;
	testSystem.paramGetByName( "D" )->value = 2.0;
	testSystem.paramGetByName( "E" )->value = 0.0;
	testSystem.paramGetByName( "c" )->value = 1.0;

	testSystem.clrFitFlags();
	testSystem.setFitFlags( PI_REACTION_RATE );
	KineticParameterInfo *pi;
	pi = testSystem.paramGetByName( "A" ); pi->fitFlag = 1;
	pi = testSystem.paramGetByName( "D" ); pi->fitFlag = 1;
	pi = testSystem.paramGetByName( "c" ); pi->fitFlag = 1;
	return 0;
}


int setupTest31( int modify ) {
	setupCallback = setupTest31;
	// A->B, 1 experiments, 1 observables, modify 1 rate consts, fit 1 rate const
	// Test of exponential anlytic fit #3

	if( modify ) {
		testSystem.paramSetReactionRate( 0, startSystem.paramGetReactionRate(0) * 0.5 );
		testSystem.paramSetReactionRate( 1, startSystem.paramGetReactionRate(1) * 0.5 );
		testSystem.paramSetReactionRate( 2, startSystem.paramGetReactionRate(2) * 0.5 );
		testSystem.paramGetByName( "A" )->value *= 0.5;
		testSystem.paramGetByName( "D" )->value *= 0.5;
		testSystem.paramGetByName( "F" )->value *= 0.5;
		testSystem.paramGetByName( "c" )->value *= 0.5;
		return ZF_DONE_CONVERGED;
	}
	...
	testSystem.reagentAdd( "D" );
	testSystem.reagentAdd( "E" );
	testSystem.reagentAdd( "F" );
	testSystem.reagentAdd( "G" );
	...
	KineticExperiment *e = testSystem.newExperiment();
	e->mixsteps[0].duration = 6.0;

	e->observableInstructions.add( strdup("A+D+F+c") );

	testSystem.allocParameterInfo();
	testSystem.compile();

	testSystem.paramSetReactionRate( 0, 10 );
	testSystem.paramSetReactionRate( 1, 3 );
	testSystem.paramSetReactionRate( 2, 0.5 );
	testSystem.paramGetByName( "A" )->value = 1.0;
	testSystem.paramGetByName( "B" )->value = 0.0;
	testSystem.paramGetByName( "D" )->value = 2.0;
	testSystem.paramGetByName( "E" )->value = 0.0;
	testSystem.paramGetByName( "F" )->value = 3.0;
	testSystem.paramGetByName( "G" )->value = 0.0;
	testSystem.paramGetByName( "c" )->value = 1.0;
	...
	pi = testSystem.paramGetByName( "A" ); pi->fitFlag = 1;
	pi = testSystem.paramGetByName( "D" ); pi->fitFlag = 1;
	pi = testSystem.paramGetByName( "F" ); pi->fitFlag = 1;
	pi = testSystem.paramGetByName( "c" ); pi->fitFlag = 1;
	return 0;
}

int setupTest32( int modify ) {
	setupCallback = setupTest32;
	// A->B, 1 experiments, 1 observables, modify 1 rate consts, fit 1 rate const
	// Test of exponential anlytic fit #4
...
/*
		...
		testSystem.paramSetReactionRate( 1, startSystem.paramGetReactionRate(1) * 0.5 );
		testSystem.paramSetReactionRate( 2, startSystem.paramGetReactionRate(2) * 0.5 );
		testSystem.paramSetReactionRate( 3, startSystem.paramGetReactionRate(3) * 0.5 );
		testSystem.paramGetByName( "A" )->value *= 0.5;
		testSystem.paramGetByName( "D" )->value *= 0.5;
		testSystem.paramGetByName( "F" )->value *= 0.5;
		testSystem.paramGetByName( "H" )->value *= 0.5;
		testSystem.paramGetByName( "c" )->value *= 0.5;
*/
		testSystem.paramSetReactionRate( 0, startSystem.paramGetReactionRate(0) * 0.0 );
		testSystem.paramSetReactionRate( 1, startSystem.paramGetReactionRate(1) * 0.0 );
		testSystem.paramSetReactionRate( 2, startSystem.paramGetReactionRate(2) * 0.0 );
		testSystem.paramSetReactionRate( 3, startSystem.paramGetReactionRate(3) * 0.0 );
		testSystem.paramGetByName( "A" )->value *= 0.0;
		testSystem.paramGetByName( "D" )->value *= 0.0;
		testSystem.paramGetByName( "F" )->value *= 0.0;
		testSystem.paramGetByName( "H" )->value *= 0.0;
		testSystem.paramGetByName( "c" )->value *= 0.0;
	...
	testSystem.reagentAdd( "F" );
	testSystem.reagentAdd( "G" );
	testSystem.reagentAdd( "H" );
	testSystem.reagentAdd( "I" );

	testSystem.reactionAdd(  0, -1,  1, -1 );
	testSystem.reactionAdd(  2, -1,  3, -1 );
	testSystem.reactionAdd(  4, -1,  5, -1 );
	testSystem.reactionAdd(  6, -1,  7, -1 );

	KineticExperiment *e = testSystem.newExperiment();
	e->mixsteps[0].duration = 20.0;

	e->observableInstructions.add( strdup("A+D+F+H+c") );
	...
	testSystem.paramSetReactionRate( 1, 3 );
	testSystem.paramSetReactionRate( 2, 0.5 );
	testSystem.paramSetReactionRate( 3, 0.01 );
	testSystem.paramGetByName( "A" )->value = 4.0;
	testSystem.paramGetByName( "B" )->value = 0.0;
	testSystem.paramGetByName( "D" )->value = 3.0;
	testSystem.paramGetByName( "E" )->value = 0.0;
	testSystem.paramGetByName( "F" )->value = 2.0;
	testSystem.paramGetByName( "G" )->value = 0.0;
	testSystem.paramGetByName( "H" )->value = 1.0;
	testSystem.paramGetByName( "I" )->value = 0.0;
	testSystem.paramGetByName( "c" )->value = 1.0;
	...
	pi = testSystem.paramGetByName( "D" ); pi->fitFlag = 1;
	pi = testSystem.paramGetByName( "F" ); pi->fitFlag = 1;
	pi = testSystem.paramGetByName( "H" ); pi->fitFlag = 1;
	pi = testSystem.paramGetByName( "c" ); pi->fitFlag = 1;
	return 0;
}

int setupTest33( int modify ) {
	setupCallback = setupTest33;

	if( modify ) {
		testSystem.paramSetReactionRate( 0, 0.0001 );
		testSystem.paramGetByName( "A" )->value *= 0.5;
		testSystem.paramGetByName( "B" )->value *= 0.5;
		testSystem.paramGetByName( "C" )->value *= 0.5;
		testSystem.paramGetByName( "o1" )->value *= 0.5;
		testSystem.paramGetByName( "o2" )->value *= 0.5;
		return ZF_DONE_CONVERGED;
	}
	...
	e->observableInstructions.add( strdup("A+o1+o2") );
	...
	testSystem.paramSetReactionRate( 0, 10 );
	testSystem.paramGetByName( "A" )->value = 1.0;
	testSystem.paramGetByName( "B" )->value = 1.0;
	testSystem.paramGetByName( "C" )->value = 0.0;
	testSystem.paramGetByName( "o1" )->value = 1.0;
	testSystem.paramGetByName( "o2" )->value = 1.0;
...
void generateErrorSurface() {
	...
	testSystem.simulate();
	for( int i=0; i<testSystem.eCount(); i++ ) {
		testSystem[i]->measuredCreateFake( 1000, Aode2_noise );
	}
...
return;
	...
	if( renderFitter.state == ZF_DONE_CONVERGED || renderFitter.state == ZF_DONE_TRAPPED ) {

		// ASK the test what the correct result should be
		printf(
			"Test %d stop. propCount=%d, origCount=%d, Result status=%d, %s\n",
			regressionTestI,
			renderFitter.propCount(),
			renderFitter.origCount(),
			renderFitter.state,
			(*regressionTestFns[regressionTestI])(1) == renderFitter.state ? "PASS" : "FAIL"
		);
	...
	if( stateChange ) {
		...
		// LAUNCH the fitter	
		mainFitter.start( testSystem );
		fitterThread.threadStart( &mainFitter, &renderFitter, 0 );
	}
}
...
//	setupTest28(0);    // Analytic exponential #1
//	setupTest29(0);    // Same as 28 but everything starts at zero. FAILING
//	setupTest30(0);    // Analytic exponential #2
//	setupTest31(0);    // Analytic exponential #3
//	setupTest32(0);    // Analytic exponential #4
//	setupTest33(0);
	...
	char *path = options.getS( "pluginPath_aode2" );
	ZUI::zuiExecuteFile( ZTmpStr("%s/../_aode/aode.zui",path) );
}
...
void handleMsg( ZMsg *msg ) {
	...
	// mkness - run full test sequence
	if( zmsgIs(type,Key) && zmsgIs(which,K) ) {
		Aode2_noise = 0.0;
		// w
		for( i=0; i<testSystem.eCount(); i++ ) {
			testSystem[i]->measuredCreateFake( 1000, Aode2_noise );
		...
		for (int i=0; i < 100; i++)
		{
			printf ("about to do iter %d\n", i);
			mainFitter.step ();
		}
		printf ("completed 100 iterations ok.\n");
	}
	...
	if( zmsgIs(type,Key) && zmsgIs(which,r) ) {
		...
		fitterThread.threadStart( &mainFitter, &renderFitter, 1 );

//		zMsgQueue( "type=ZUISet key=fitter val=0x%x toZUI=aodeFitterPanel", &fitterThread );
//		zMsgQueue( "type=ZUISet key=*fitter val=0x%x toZUI=aodeFitterPanel", &fitterThread );
	}
	
	if( zmsgIs(type,Key) && zmsgIs(which,t) ) {
		fitterThread.threadStepFlag = 1;
	}

	if( zmsgIs(type,Key) && zmsgIs(which,T) ) {
		fitterThread.threadStepFlag = 1;
		fitterThread.threadMonitorStep = 0;
	}

	if( zmsgIs(type,Key) && zmsgIs(which,y) ) {
		renderFitter.qHistory.saveCSV( "qhistory.csv" );
		renderFitter.eHistory.saveCSV( "ehistory.csv" );
		renderFitter.stateHistory.saveCSV( "statehistory.csv" );
	}
